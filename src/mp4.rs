...
use base::{bail_t, format_err_t, strutil, Error, ErrorKind, ResultExt};
...
impl Segment {
    ...
    fn truns(
        ...
        self.s
            .foreach(playback, |it| {
                if it.is_key() {
                    ...
                    let r = run_info
                        .as_mut()
                        .expect("non-key sample must be preceded by key sample");
                    r.count += 1;
        ...
        if let Some(r) = run_info.take() {
            ...
            BigEndian::write_u32(
                &mut v[p - 8..p - 4],
                cmp::min(self.s.desired_range_90k.end - r.last_start, r.last_dur) as u32,
            );
        }
        Ok(v)
    }
...
impl Slice {
        ...
        F: Fn(&[u8], SegmentLengths) -> &[u8],
    {
        ...
        let p = self.p();
        Ok(mp4
            .try_map(|mp4| Ok::<_, Error>(&mp4.segments[p].get_index(&mp4.db, f)?[r]))?
            .into())
    }
    ...
    fn get_range(
        ...
        let res = match self.t() {
            ...
            SliceType::VideoSampleEntry => {
                let r = ARefss::new(f.0.clone());
                Ok(r.map(|f| {
                    &f.video_sample_entries[p].data[range.start as usize..range.end as usize]
                })
                .into())
            }
...
impl FileBuilder {
    ...
    pub fn build(
        ...
        let mut etag =
            hash::Hasher::new(hash::MessageDigest::sha1()).err_kind(ErrorKind::Internal)?;
        ...
        for s in &mut self.segments {
            ...
            cursor
                ...
                .err_kind(ErrorKind::Internal)?;
            cursor
                .write_i64::<BigEndian>(s.s.start.0)
                .err_kind(ErrorKind::Internal)?;
            cursor
                .write_u32::<BigEndian>(s.s.open_id)
                .err_kind(ErrorKind::Internal)?;
            cursor
                .write_i32::<BigEndian>(d.start)
                .err_kind(ErrorKind::Internal)?;
            cursor
                .write_i32::<BigEndian>(d.end)
                .err_kind(ErrorKind::Internal)?;
...
impl FileInner {
    ...
    fn get_subtitle_sample_data(&self, i: usize, r: Range<u64>, l: u64) -> Result<Chunk, Error> {
        ...
        let start_sec = (s.s.start + recording::Duration(d.start as i64)).unix_seconds();
        let end_sec =
            (s.s.start + recording::Duration(d.end as i64 + TIME_UNITS_PER_SEC - 1)).unix_seconds();
        let mut v = Vec::with_capacity(l as usize);
...
mod tests {
        ...
        E::Error: ::std::fmt::Debug,
    {
        Pin::from(e.get_range(0..e.len()))
            .try_fold(
                hash::Hasher::new(hash::MessageDigest::sha1()).unwrap(),
                |mut sha1, chunk| {
                    let c: &[u8] = chunk.bytes();
    ...
    async fn test_half_sync_frames() {
        ...
        let mp4 =
            make_mp4_from_encoders(Type::Normal, &db, vec![r], 2 + 4 + 6..2 + 4 + 6 + 8).unwrap();
        let track = find_track(mp4, 1).await;
    ...
    async fn test_no_segments() {
        testutil::init();
        let db = TestDb::new(RealClocks {});
        let e = make_mp4_from_encoders(Type::Normal, &db, vec![], 0..0)
            .err()
            .unwrap();
    ...
    async fn test_multi_segment() {
        ...
        let mp4 =
            make_mp4_from_encoders(Type::Normal, &db, encoders, 1 + 2..1 + 2 + 3 + 4).unwrap();
        let mut cursor = BoxCursor::new(mp4);
        cursor.down().await;
        assert!(cursor.find(b"moov").await);
        cursor.down().await;
    ...
    async fn test_round_trip() {
        ...
        assert_eq!(
            "17376879bcf872dd4ad1197225a32d5473fb0dc6",
            strutil::hex(&sha1[..])
        );
    ...
    async fn test_round_trip_with_subtitles() {
        ...
        let sha1 = digest(&mp4).await;
        assert_eq!(
            "1cd90e0b49747cc54c953153d6709f2fb5df6b14",
            strutil::hex(&sha1[..])
        );
    ...
    async fn test_round_trip_with_edit_list() {
        ...
        let sha1 = digest(&mp4).await;
        assert_eq!(
            "49893e3997da6bc625a04b09abf4b1ddbe0bc85d",
            strutil::hex(&sha1[..])
        );
    ...
    async fn test_round_trip_with_shorten() {
        ...
        let sha1 = digest(&mp4).await;
        assert_eq!(
            "0615feaa3c50a7889fb0e6842de3bd3d3143bc78",
            strutil::hex(&sha1[..])
        );
