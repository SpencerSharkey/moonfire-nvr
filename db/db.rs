...
impl Stream {
    ...
    pub fn days(&self) -> BTreeMap<StreamDayKey, StreamDayValue> {
        ...
        for u in &self.uncommitted {
            let l = u.lock();
            adjust_days(
                l.start..l.start + recording::Duration(i64::from(l.duration_90k)),
                1,
                &mut days,
            );
        }
...
impl LockedDatabase {
    ...
    fn flush<C: Clocks>(&mut self, clocks: &C, reason: &str) -> Result<(), Error> {
        ...
        for (stream_id, new_range) in new_ranges.drain() {
            ...
            for _ in 0..s.synced_recordings {
                ...
                log.added
                    .push(CompositeId::new(stream_id, s.next_recording_id));
    ...
    pub fn list_recordings_by_id(
        ...
        if desired_ids.end > s.next_recording_id {
            ...
            let end = cmp::min(
                (desired_ids.end - s.next_recording_id) as usize,
                s.uncommitted.len(),
            );
            for i in start..end {
                let row = {
        ...
        self.list_recordings_by_time(stream_id, desired_time, &mut |row| {
            ...
            match aggs.entry(run_start_id) {
                Entry::Occupied(mut e) => {
                    let a = e.get_mut();
                    let new_dur =
                        a.time.end - a.time.start + recording::Duration(row.duration_90k as i64);
        ...
        if s.next_recording_id <= id.recording() {
            ...
            if i as usize >= s.uncommitted.len() {
                bail!(
                    ...
                    id,
                    s.next_recording_id,
                    s.next_recording_id + s.uncommitted.len() as i32
                );
            }
    ...
    pub fn delete_sample_file_dir(&mut self, dir_id: i32) -> Result<(), Error> {
        ...
        meta.in_progress_open = mem::replace(
            &mut meta.last_complete_open,
...
mod tests {
    ...
    fn test_version_too_old() {
        testutil::init();
        let c = setup_conn();
        c.execute_batch("delete from version; insert into version values (4, 0, '');")
            .unwrap();
        let e = Database::new(clock::RealClocks {}, c, false).err().unwrap();
        assert!(
            e.to_string()
                .starts_with("Database schema version 4 is too old (expected 5)"),
            "got: {:?}",
            e
        );
    }
    ...
    fn test_version_too_new() {
        ...
        assert!(
            e.to_string()
                .starts_with("Database schema version 6 is too new (expected 5)"),
            "got: {:?}",
            e
        );
    }
    ...
    fn test_full_lifecycle() {
        ...
        let vse_id = db
            ...
            .insert_video_sample_entry(
                1920,
                1080,
                include_bytes!("testdata/avc1").to_vec(),
                "avc1.4d0029".to_owned(),
            )
            .unwrap();
